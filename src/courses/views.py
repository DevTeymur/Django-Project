...
def course_list(request, category_slug=None, tag_slug=None):
    ...
    if category_slug!=None:
        ...
        if current_user.is_authenticated:
            enroll_courses=current_user.courses_joined.all()
    ...
    return render(request, 'courses.html', context)   #render send context to courses.html
...
def tag_list(request, tag_slug):
    courses = Course.objects.all().filter(tags__slug=tag_slug)  # tags from model.py !!!
    categories = Category.objects.all()
    tags = Tag.objects.all()
    context = {
        'courses' : courses,
